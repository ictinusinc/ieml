(function(w) {
	...
	window.array_fill = function(len, val) {
	...
	window.ieml_render_table_header = function(heads, render_callback) {
		var out = '';
	...
	window.ieml_render_table_body = function(info, render_callback) {
		...
		for (var r=0; r<body.length; r++) {
			...
			if (heads) {
				for (var h = heads[0].length - 1; h >= 0; h--) {
					if (hor_tally[h][0] === 0) {
						out += '<th rowspan="' + Math.max(1, heads[0][h][hor_tally[h][1]][1]) + '">' + render_callback(heads[0][h][hor_tally[h][1]][0]) + '</th>';
					}
			...
			for (var c=0; c<body[r].length; c++) {
				out += '<td>' + render_callback(body[r][c]) + '</td>';
			}
	...
	window.ieml_render_table = function(info, render_callback) {
		var out = '';
		
		out += '<table class="relation"><tbody>';
		
		if (info.hor_header_depth > 0) {
			out += '<tr>' + '<th class="empty_cell" rowspan="' + (info.ver_header_depth + 1) + '" ' + (info.hor_header_depth > 1 ? 'colspan="' + info.hor_header_depth + '"' : '') + '>&nbsp;</th>' + '</tr>';
		}
